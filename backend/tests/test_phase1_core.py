...
def test_scoring_and_insufficient_status(subject_asset):
    ...
    score_tuple = build_scoring_tuple(Candidate(), subject_asset, 0.1)
    ranked = rank_eligible_candidates([{"id": 1, "score_tuple": score_tuple}], limit=1)
...
def test_final3_selection_and_stub_extraction(session, project, subject_asset):
    ...
    with pytest.raises(InvalidFinal3Selection):
        select_final3(session, project.id, [records[0].id, records[0].id], "analyst_a")
