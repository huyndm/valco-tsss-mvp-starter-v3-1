...
def select_final3(session: Session, project_id: int, eligible_candidate_ids: list[int], selected_by: str, override_reason: str | None = None) -> list[Final3Selection]:
    ...
    if len(eligible_candidate_ids) > settings.final_main_count:
        raise InvalidFinal3Selection(f"Cannot select more than {settings.final_main_count} main TSSS")
    ...
    session.add(AuditLog(project_id=project_id, action="final3_selection", actor=selected_by, details_json=json.dumps({"eligible_candidate_ids": eligible_candidate_ids, "override_reason": override_reason}), created_at=datetime.utcnow()))
    session.commit()
